...
import dnnlib
from gradio_utils import (ImageMask, draw_points_on_image,
...
def update_image_draw(image, points, mask, global_state=None):
    image_draw = draw_points_on_image(image, points)
    if mask is not None and not (mask == 0).all() and not (
            mask == 1).all():
...
with gr.Blocks() as app:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=4, min_width=10):
                        form_smile_slider = gr.Slider(minimum=-1.0, maximum=1.0, step=0.01,
                                                      value=0.0, label="Smile Ratio")
                        form_face_slimming_slider = gr.Slider(minimum=-1.0, maximum=1.0, step=0.01,
                                                              value=0.0, label="Face slimming Ratio")
                        form_eye_slider = gr.Slider(minimum=-1.0, maximum=1.0, step=0.01,
                                                    value=0.0, label="Eye Ratio")

                # Process
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='Process', show_label=False)
                    with gr.Column(scale=4, min_width=10):
                        with gr.Row():
                            with gr.Column(scale=1, min_width=10):
    ...
    gr.HTML("""
        ...
        <a href="httpshps://github.com/open-mmlab/mmagic">OpenMMLab MMagic</a>
        </div>
        """)
    ...
    form_start_btn.click(
        ...
        outputs=[
            ...
            # >>> buttons
            form_reset_image,
            # Removed enable_add_mask
            undo_points,
            # Removed form_reset_mask_btn
            form_latent_space,
            form_start_btn,
            form_stop_btn,
            # <<< buttonm
    ...
    def on_click_reset_mask(global_state):
        ...
        image_draw = update_image_draw(global_state['images']['image_raw'],
                                       global_state['points'],
                                       global_state['mask'], global_state)
    ...
    def on_change_smile_ratio(global_state, new_value):
        ...
        global_state['smile_ratio'] = new_value

        points = global_state["points"]
        point_idx = get_latest_points_pair(points)

        image_np = np.array(global_state['images']['image_raw'])
        preds = fa.get_landmarks(image_np)

        if preds:
            last_array = preds[-1]
            left_corner = last_array[48].astype(int)
            right_corner = last_array[54].astype(int)

            # Calculate new positions for the corners based on smile ratio
            mid_x = (left_corner[0] + right_corner[0]) / 2
            mid_y = (left_corner[1] + right_corner[1]) / 2

            distance_x = (right_corner[0] - left_corner[0]) * (1 + new_value)
            distance_y = (right_corner[1] - left_corner[1]) * (1 + new_value)

            new_left_corner = np.array((mid_x - distance_x / 2, mid_y - distance_y / 2)).astype(int)
            new_right_corner = np.array((mid_x + distance_x / 2, mid_y - distance_y / 2)).astype(int)

            if point_idx is None:
                points[0] = {'start': right_corner, 'target': new_right_corner}
                print(f'start - {right_corner}')
                print(f'target - {new_right_corner}')
                print(f'Add point - Smile - 0')
            else:
                points[point_idx + 1] = {'start': right_corner, 'target': new_right_corner}
                print(f'Add point - Smile - {point_idx + 1}')
    ...
    form_smile_slider.change(
        on_change_smile_ratio,
        inputs=[global_state, form_smile_slider],
        outputs=[global_state, form_image],
    )


    def on_change_face_ratio(global_state, new_value):
        ...
        global_state['face_slimming_ratio'] = new_value
        ...
        if preds:
            last_array = preds[-1]
            points_0_to_2 = last_array[0:3].astype(int)
            points_14_to_16 = last_array[14:17].astype(int)

            points_start = []
            points_end = []

            for i in range(3):
                left_corner = points_0_to_2[i]
                right_corner = points_14_to_16[2 - i]

                mid_x = (left_corner[0] + right_corner[0]) / 2
                distance_x = abs(right_corner[0] - left_corner[0])

                new_left_x = mid_x - distance_x * (1 - new_value) / 2
                new_right_x = mid_x + distance_x * (1 - new_value) / 2

                new_left_corner = np.array((new_left_x, left_corner[1])).astype(int)
                new_right_corner = np.array((new_right_x, right_corner[1])).astype(int)

                points_start.append(left_corner)
                points_start.append(right_corner)
                points_end.append(new_left_corner)
                points_end.append(new_right_corner)

            for i, (new_start, new_end) in enumerate(zip(points_start, points_end)):
                if point_idx is None:
                    points[i] = {'start': new_start, 'target': new_end}
                    print(f'start - {new_start}')
                    print(f'target - {new_end}')
                    print(f'Add point - Face - 0')
                else:
                    points[point_idx + i + 1] = {'start': new_start, 'target': new_end}
                    print(f'Add point - Face - {point_idx + i + 1}')
    ...
    form_face_slimming_slider.change(
        on_change_face_ratio,
        inputs=[global_state, form_face_slimming_slider],
        outputs=[global_state, form_image],
    )


    def on_change_eye_ratio(global_state, new_value):
        ...
        global_state['eye_ratio'] = new_value
        ...
        if preds:
            last_array = preds[-1]
            points_37_to_44 = last_array[[37, 38, 43, 44]].astype(int)
            points_41_to_46 = last_array[[41, 40, 47, 46]].astype(int)

            points_start = []
            points_end = []

            for i in range(4):
                upper_coord = points_37_to_44[i]
                lower_coord = points_41_to_46[i]

                mid_y = (upper_coord[1] + lower_coord[1]) / 2
                distance_y = abs(lower_coord[1] - upper_coord[1])

                new_upper_y = mid_y - distance_y * (1 + new_value) / 2
                new_lower_y = mid_y + distance_y * (1 + new_value) / 2

                new_upper_coord = np.array((upper_coord[0], new_upper_y)).astype(int)
                new_lower_coord = np.array((lower_coord[0], new_lower_y)).astype(int)

                points_start.append(upper_coord)
                points_start.append(lower_coord)
                points_end.append(new_upper_coord)
                points_end.append(new_lower_coord)
            ...
            for i, (new_start, new_end) in enumerate(zip(points_start, points_end)):
                if point_idx is None:
                    ...
                    print(f'start - {new_start}')
                    print(f'target - {new_end}')
                    print(f'Add point - Eye - 0')
                else:
                    points[point_idx + i + 1] = {'start': new_start, 'target': new_end}
                    print(f'Add point - Eye - {point_idx + i + 1}')
    ...
    form_eye_slider.change(
        on_change_eye_ratio,
        inputs=[global_state, form_eye_slider],
        outputs=[global_state, form_image],
    )
